//
//  CNJSEssenceTopicTableViewController.m
//  DNApp
//
...
#import "CNJSEssenceTopicTableViewController.h"
#import "CNJSManger.h"
#import "TopicModel.h"
...
@interface CNJSEssenceTopicTableViewController ()
@property (nonatomic,strong) NSMutableArray *topics;
@property (nonatomic) int cur_page;
@end

@implementation CNJSEssenceTopicTableViewController
...
-(void)PullRefresh{
    RACCommand *refreshCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
        return [[CNJSManger sharedManager] getShareTopicsatPage:1];
    }];
...
-(void)LoadMore{
    ...
    RACCommand *loadMoreCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input){
        return [[CNJSManger sharedManager] getShareTopicsatPage:self.cur_page];
    }];
...
-(void)LoadTopicById:(NSString*)topicId{
    ...
    [[getTopic.executionSignals flatten] subscribeNext:^(TopicModel* topic){
        [self performSegueWithIdentifier:@"goodToDetail" sender:topic];
    }];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self performSegueWithIdentifier:@"goodToDetail" sender:model];
}
